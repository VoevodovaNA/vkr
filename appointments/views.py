    ...
    if request.method == "POST":
        ...
        if registration_form.is_valid():  # get data from form (if it is valid)
            ...
            if dob < timezone.now().date():  # check if date of birth is valid (happened the previous day or even back)
                ...
                messages.add_message(request, messages.INFO, 'Регистрация успешна!')
...
def dashboard_adm_view(request):
    if check_admin(request.user):
        ...
        messages.add_message(request, messages.INFO, 'Есть {0} заявок, требующих одобрения.'.format(pending_app_total))
...
def approve_eng_adm_action(request, pk):
    if check_admin(request.user):
        ...
        messages.add_message(request, messages.INFO, 'Мастер успешно одобрен.')
        ...
        if request.method == 'POST':
            ...
            if feedback_form.is_valid():  # if form is valid
                ...
                try:
                    ...
                    messages.add_message(request, messages.INFO, 'Спасибо, что оставили свой отзыв.')
...
def check_eng_availability(engineer: object, dt: object, tm: object) -> object:  # check if engineer is available in a given slot
...
@csrf_exempt
def send_data_to_external_system(request):
    if request.method == 'POST':
        data = request.POST  # Получаем данные из POST запроса

        # Формируем данные для отправки на внешний сервер (пример)
        payload = {
            'key1': data.get('key1'),
            'key2': data.get('key2')
        }

        # URL внешней системы, куда будем отправлять данные
        external_system_url = 'https://external-system.com/api/endpoint'

        try:
            # Отправляем данные на внешний сервер
            response = requests.post(external_system_url, data=payload)
